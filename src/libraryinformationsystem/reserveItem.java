 ...
 */
public class reserveItem extends JFrame {
	...
	private final String Msg_PleaseSelectedABookToBorrow="Please select an item to borrow!";
	...
	public reserveItem(Library library, Patron customer) {
		...
		pnlBooks = new JPanel();
		pnlBooks.setBorder(new TitledBorder(null, "Items", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		btnBorrow.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (reserveItem.this.library==null)
					return;
				
				 //Borrow  selected book 
				 Item item = getSelectedBook();
				 if (item==null) {
					JOptionPane.showMessageDialog(null, Msg_PleaseSelectedABookToBorrow);
					return;
				}
				
				boolean suc = reserveItem.this.library.rentItem(reserveItem.this.customer.getUserId(), item.getIsbn());
				itemFinish itemReserved = new itemFinish(suc,item);
				itemReserved.setModal(true);
				itemReserved.setVisible(true);
				reserveItem.this.refreshBookTableUI();
			}	
		});
	...
	/**
	 * Set whether will the TbBooksSelectedChanged event handler will be triggered
	 */
	...
	private Object[] createBookTableRowData(Item book) {
	...
	private Object[][] getAvailableBookTableData() {
		if (this.library==null)
			...
			List<Item> books =library.showItemList_remainder();
	...
	private Object[][] getNewBookTableData(Category[] subScribeCategories) {
		if (this.library==null)
			...
			List<Item> books =library.showItemList_new(subScribeCategories);
			int nBooks = books.size();//number of books
			Object[][] booksData = new Object[nBooks][];
			for (int i=0;i<nBooks;i++) {
				booksData[i]=createBookTableRowData(books.get(i));
	...
	private Object[][] getAllBookTableData() {
		if (this.library==null)
			...
			List<Item> items =library.showItemList_all();
			int nBooks = items.size();//number of books
			Object[][] booksData = new Object[nBooks][];
			for (int i=0;i<nBooks;i++) {
				booksData[i]=createBookTableRowData(items.get(i));
			}
